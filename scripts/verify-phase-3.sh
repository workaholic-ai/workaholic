...
if [ "$#" -ne 0 ]; then
  printf '%s\n' "usage: scripts/verify-phase-3.sh" >&2
...
verify_clean_worktree() {
  ...
  if [ -n "$verify_dirty_paths" ]; then
    printf '%s\n' "phase-three: verification requires a clean Git worktree" >&2
...
verify_runtime_directory=$(
  mktemp -d "${TMPDIR:-/tmp}/workaholic-phase-three-gate.XXXXXX"
)
...
printf '%s\n' "[6/6] Verifying the installed Phase 3 Human lifecycle"
scripts/smoke-phase-3-wheel.sh dist/*.whl

verify_clean_worktree
printf '%s\n' "Phase 3 clean-state acceptance gate passed."
