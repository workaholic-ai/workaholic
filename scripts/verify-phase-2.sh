...
if [ "$#" -ne 0 ]; then
  printf '%s\n' "usage: scripts/verify-phase-2.sh" >&2
...
verify_clean_worktree() {
  ...
  if [ -n "$verify_dirty_paths" ]; then
    printf '%s\n' "phase-two: verification requires a clean Git worktree" >&2
...
verify_runtime_directory=$(
  mktemp -d "${TMPDIR:-/tmp}/workaholic-phase-two-gate.XXXXXX"
)
mkdir -p \
...
printf '%s\n' "[6/6] Verifying the installed Phase 2 multi-Project journey"
scripts/smoke-phase-2-wheel.sh dist/*.whl

verify_clean_worktree
printf '%s\n' "Phase 2 clean-state acceptance gate passed."
