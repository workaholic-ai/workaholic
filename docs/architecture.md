...
The file is generated by Phase 1 `workaholic up` and, from Phase 2, by:
...
Phase 1 resolves only the exact path
...
Phase 2 extends this baseline with upward discovery and configurable trusted
profiles using the full resolution order above. The repository-local file
remains untrusted and never controls credentials or endpoints.
...
Phase 1 does not create a Token, use an operating-system credential store, or
write a user profile. The embedded process selects the bootstrap Subject from
the initialized local store and still applies application-level authorization
and Task-event attribution. The filesystem is part of the Phase 1 security
boundary.
...
Phase 1 establishes the public automation contract for `up`, `status`,
`project list`, `task add`, `task list`, and `task show`, including JSON-only
stdout, non-interactive operation, stable error identifiers, exit-code
categories, and optional idempotency for the two mutations. Phase 4 extends
that contract with agent-execution commands and large structured inputs.
